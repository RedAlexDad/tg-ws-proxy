...
### 13.2. Что нового по функционалу

#### Пул WS-соединений (главное изменение)

| Что | Было | Стало |
|---|---|---|
| Фронтинг | разовый fallback в обработчике клиента, если WS таймаутнул | встроен в наполнение пула: пул сам чередует обычное соединение и фронтинг (SNI `sprinthost.ru`) |
| Ротация | не было | отдельная задача следит за возрастом соединений (120 с), закрывает протухшие и дозаполняет пул заранее |
| Refill backoff | при неудаче пул сразу долбил IP | пауза растёт экспоненциально: 1 → 2 → 4 … до 3600 с, сбрасывается при успехе |
| Выбор из пула при таймауте | таймаут прямого connect → сразу fallback | если в пуле есть готовое соединение, берётся оно |
| Refill после ротации | — | после закрытия протухших соединений пул сразу дозаполняется |

Наглядно в логах:

```
WS pool refill failed for DC2, retry in 1s
DC2 WS connect to 149.154.167.220 was timed out, but pool hit -> using WS
DC2m WS session closed (normal): ^2.9KB (7 pkts) v22.9KB (7 pkts) in 16.1s
```
...
| Что | Было | Стало |
|---|---|---|
| Хранилище | соединения не кэшировались | пул CF-worker соединений на каждый DC (TTL 100 с) + статистика hit/miss |
| Лимит | — | не больше 1 worker-соединения на DC |
| Отключение | — | воркер при ошибке 429 отключается до конца суток (заготовка, пока `TODO`) |

#### Прочее

- **Фрагментация WS** (`raw_websocket`): сборка fragmented-сообщений
  (continuation-фреймы, лимит 16 МБ) — раньше неполные кадры могли
  теряться.
- **Цензура доменов в логах** (`DomainCensorFilter`): CF-домены из списка
  маскируются (`pc****.co.uk`), `*.telegram.org` остаются читаемыми.
- **Тестовые DC**: флаг `force_test_dc` и поддержка тестового окружения
  Telegram.
- **SSL-контекст через certifi** для запросов к GitHub (обновление списка
  CF-доменов).
- **Сохранение пользовательских CF-доменов** вместо затирания.
- **Багфиксы**: #1113 (лишний `path` в `ws_pool.get`), #1161/#1155
  (splitter в CF-пути), #1232 (фронтинг перестал «залипать»), проверка IP
  через `inet_pton` вместо устаревшего `inet_aton`.

### 13.3. Хронология черри-пика
